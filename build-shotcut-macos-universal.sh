...
function set_globals {
  ...
  if test "$TARGET_OS" = "Darwin"; then
    [ "$QTDIR" = "" ] && export QTDIR="/opt/custom/qt/$QT_VERSION_DARWIN/macos"
    export RANLIB=ranlib
  else
    if test -z "$QTDIR" ; then
      if [ "$(uname -m)" = "x86_64" ]; then
        export QTDIR="/opt/custom/qt/$QT_VERSION_DEFAULT/gcc_64"
      else
        export QTDIR="/opt/custom/qt/$QT_VERSION_DEFAULT/gcc"
      fi
    fi
    export RANLIB=ranlib
  fi
  ...
  #####
  # ffmpeg
  CONFIG[0]="./configure --disable-static --disable-doc --enable-gpl --enable-version3 --enable-shared --enable-runtime-cpudetect $CONFIGURE_DEBUG_FLAG --extra-cflags=-I/opt/local/include --extra-ldflags=-L/opt/local/lib"
  ...
  #####
  # opencv
  CONFIG[26]="cmake -B build -G Ninja -D CMAKE_INSTALL_PREFIX=$FINAL_INSTALL_DIR -D BUILD_LIST=tracking -D OPENCV_GENERATE_PKGCONFIG=YES -D OPENCV_EXTRA_MODULES_PATH=../opencv_contrib/modules -D WITH_OPENMP=ON -DWITH_IPP=OFF $CMAKE_DEBUG_FLAG"
...
function build_ffmpeg_darwin {
  ...
  cmd $MYCONFIG --prefix=build-x86_64 --enable-x86asm
  cmd make -j $MAKEJ
  cmd make install
  cmd make distclean
  ASFLAGS="-arch arm64"
  cmd $MYCONFIG --prefix=build-arm64 --enable-cross-compile --arch=arm64  --cc='clang -arch arm64'
  cmd make -j $MAKEJ
  cmd make install
}
...
#  if [ "$1" = "libwebp" ]; then
#    sed -i "" "s/-mno-sse4.1;-mno-sse2;;-mno-dspr2;;-mno-msa/-;;;-mno-dspr2;;-mno-msa/g" $1/cmake/cpu.cmake
#  fi
...
export MLT_MOVIT_PATH="/opt/local/share/movit/"
